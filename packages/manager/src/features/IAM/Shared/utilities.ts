...
export const mergeAssignedRolesIntoExistingRoles = (
  ...
  selectedAccountAccessRoles.forEach((r) => {
    ...
    if (!selectedPlusExistingRoles.account_access.includes(r.role?.value as AccountAccessRole)) {
      selectedPlusExistingRoles.account_access.push(r.role?.value as AccountAccessRole);
    }
  });
